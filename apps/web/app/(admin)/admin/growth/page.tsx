...
export default async function AdminGrowthPage({
  ...
  const [overview, regionRows, cityPresence, heatmap, heatmapCities, regions] = await Promise.all([
    getAdminGrowthOverviewAction(),
    getAdminRegionGrowthRowsAction(),
    getAdminCityPresenceAction(),
  ...
  return (
    <div>
      ...
      {/* PRESENÇA REAL — derivada dos perfis, não de FK estratégica */}
      <section className="mb-8">
        <h2 className="mb-1 flex items-center gap-2 text-sm font-semibold text-foreground">
          <Users className="size-4" />
          Presença real por cidade
        </h2>
        <p className="mb-3 text-xs text-muted-foreground">
          Contagem direta dos perfis cadastrados (cidade normalizada do próprio
          perfil). Independe de região estratégica — uma cidade com usuários
          reais aparece aqui mesmo sem região cadastrada.
        </p>

        {cityPresence.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
            Nenhum perfil com cidade cadastrada ainda.
          </div>
        ) : (
          <div className="overflow-x-auto rounded-2xl border border-border">
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-muted/40">
                <tr>
                  {["Cidade", "UF", "Profissionais", "Tutores", "Solicitações", "Região estratégica"].map((h) => (
                    <th
                      key={h}
              ...
              <tbody className="divide-y divide-border">
                {cityPresence.map((row) => (
                  <tr key={`${row.city}-${row.state}`} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-3 text-xs">
                      {row.hasStrategicRegion ? (
                        <span className="text-green-600 dark:text-green-400">cadastrada</span>
                      ) : (
      ...
      {/* Tabela de regiões — camada ESTRATÉGICA */}
      <section className="mb-8">
        <h2 className="mb-1 flex items-center gap-2 text-sm font-semibold text-foreground">
          <TrendingUp className="size-4" />
          Regiões estratégicas — ordenadas por índice de saúde
        </h2>
        <p className="mb-3 text-xs text-muted-foreground">
          Camada de planejamento. Conta apenas perfis explicitamente associados a
          uma região ou bairro. Enquanto essa associação não for populada, os
          números aqui podem ficar zerados mesmo com presença real na cidade —
          use a tabela acima para presença.
        </p>
