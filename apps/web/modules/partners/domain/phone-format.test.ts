/**
 * Testes focados — máscara de telefone BR para Partner (GATE-8-PARTNER-INPUT-MASKS-001).
 *
 ...
 *
 * Só funções puras — nenhum acesso a banco, rede ou Next.js. Os casos cobrem
 * exatamente os cenários que a missão pediu para auditar: digitação
 * progressiva, backspace, colar formatado, colar só dígitos, edição no meio
 * da string, e o teto de dígitos de um telefone BR com DDD.
 */
...
describe("formatBrazilianPhone — teto de dígitos", () => {
  it("dígitos além do 11º são descartados, nunca truncados de forma inconsistente", () => {
    assert.equal(formatBrazilianPhone("119999999999999"), "(11) 99999-9999")
  })

  it("truncar não corta no meio de um grupo — sempre o mesmo formato de 11 dígitos", () => {
    const comDoze = formatBrazilianPhone("119999999999")
    const comOnze = formatBrazilianPhone("11999999999")
    assert.equal(comDoze, comOnze)
  })
})
